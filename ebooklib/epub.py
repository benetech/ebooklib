...
class EpubNav(EpubHtml):
    ...
    def __init__(
        self, uid="nav", file_name="nav.xhtml", media_type="application/xhtml+xml"
    ):
        super(EpubNav, self).__init__(
            uid=uid, file_name=file_name, media_type=media_type
        )
...
class EpubWriter(object):
    ...
    def write(self, out=None):
        if out is not None:
            self.out = zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED)
        else:
    ...
    def _load(self):
        try:
            self.zf = zipfile.ZipFile(
                self.file_name, "r", compression=zipfile.ZIP_DEFLATED, allowZip64=True
            )
...
def write_epub(name, book, options=None, out=None):
    """
